...
func setup_map_room_column(col_start_index: int):
	...
	turn_off_next_col_rooms(col_start_index)
	await get_tree().process_frame
	link_col_rooms_to_next_col(col_start_index, 2)
	await get_tree().process_frame
	#prune_rooms_with_no_fwd_links(col_start_index)
	await get_tree().process_frame
